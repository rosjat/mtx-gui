...
class MediumChanger(SCSI):
    ...
    def load(self, data_transfer_element, storage_element):
        _fmte, _fdte, _fse = self.get_element_offsets(
            self.medium_transport_elements, self.data_slots, self.storage_slots
        )
        ...
        r = self.movemedium(_fmte, storage_element, data_transfer_element).result
    ...
    def unload(self, data_transfer_element, storage_element):
        ...
        r = self.movemedium(
            _fmte, data_transfer_element + _fdte, storage_element + _fse - _fdte
        ).result
        modul_logger.debug(r)
